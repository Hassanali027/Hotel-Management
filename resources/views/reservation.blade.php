...
.thead,.trow{display:grid;grid-template-columns:1.4fr 1fr 1.1fr .9fr 1.7fr 1fr 1.5fr;align-items:center;min-width:1150px}
...
.ib{width:38px;height:38px;border:1px solid #ededed;background:#fff;border-radius:9px;display:grid;place-items:center;cursor:pointer}
.ib svg{width:18px;height:18px;fill:none;stroke:#555;stroke-width:1.7}
.abtn{height:38px;border:0;border-radius:9px;padding:0 18px;font-size:14px;font-weight:700;cursor:pointer}
    ...
    <section class="panel">
        <div class="pt">
            ...
            <div class="pt-r">
                ...
                <select class="fsel" id="fStatus"><option value="">All Status</option><option value="confirmed">Confirmed</option><option value="pending">Pending</option></select>
...
function render(list){
 document.getElementById('rows').innerHTML=list.map(b=>{
  const action=b.status==='pending'?`<button class="abtn confirm" onclick="post('/bookings/${b.id}/confirm','POST')">Confirm</button>`:`<button class="abtn cancel" onclick="if(confirm('Cancel this booking?'))post('/bookings/${b.id}','DELETE')">Cancel</button>`;
  const stl=b.status==='pending'?'Pending':'Confirmed';
  return `<div class="trow"><span class="g"><b>${b.guest_name}</b><small>${b.code}</small></span><span>${b.room_label||''}</span><span>${b.request||''}</span><span>${b.duration||''}</span><span>${fmt(b.check_in)} &nbsp;-&nbsp; ${fmt(b.check_out)}</span><span><em class="st ${b.status}">${stl}</em></span><span class="act"><button class="ib" title="View guest profile" onclick="location.href='/guest-profile?id=${b.id}'">${eye}</button><button class="ib" title="View guest profile" onclick="location.href='/guest-profile?id=${b.id}'">${edit}</button>${action}</span></div>`;
